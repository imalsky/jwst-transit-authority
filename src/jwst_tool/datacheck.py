...
def exomolop_table_path(mol: str) -> Path | None:
    """The ExoMolOP k-table file the DEFAULT opacity_mode reads for ``mol``
    (None when the engine config is unavailable). Filename convention is the
    engine's ``exomolop.table_path``; a data-gated test pins the two agree."""
    cfg = _engine_config()
    if isinstance(cfg, Exception):
        return None
